...
@bot.event
async def on_ready():
    # 버튼이 서버 재시작 후에도 작동하도록 등록 (Persistent Views)
    bot.add_view(PurchaseView())
    bot.add_view(CloseTicketView())
    bot.add_view(ReviewView())
    print(f'---------------------------------')
    print(f'봇 이름: {bot.user.name}')
    print(f'상태: 온라인 (Render 배포 완료)')
    print(f'---------------------------------')
...
if __name__ == "__main__":
    # 1. 웹 서버 실행 (Uptime 보장용)
    keep_alive()
    
    # 2. Render 환경 변수에서 토큰 가져오기
    token = os.environ.get('TOKEN')
    
    if token:
        bot.run(token)
    else:
        print("에러: Render 'Environment' 설정에서 TOKEN을 추가해주세요.")
