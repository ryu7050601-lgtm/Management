...
# --- [웹 서버 설정] Render의 포트 바인딩 및 Uptime용 ---
app = Flask('')
...
def run():
    # Render는 환경 변수로 'PORT'를 제공합니다. 없을 경우 8080을 사용합니다.
...
# 봇 클래스 정의 (Persistent Views 등록을 위함)
class MyBot(commands.Bot):
    def __init__(self):
        super().__init__(command_prefix="!", intents=intents, help_command=None)

    async def setup_hook(self):
        # 봇이 재시작되어도 버튼이 작동하게 함
...
class PurchaseView(discord.ui.View):
    def __init__(self):
        super().__init__(timeout=None) # timeout=None이 있어야 버튼이 안 죽음
...
async def on_ready():
    ...
    print(f'상태: 온라인 (Render 포트 바인딩 완료)')
    print(f'---------------------------------')
...
if __name__ == "__main__":
    ...
    # Render 설정에서 'TOKEN'이라는 이름으로 환경변수를 등록하세요.
